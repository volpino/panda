...
struct range_set {
    ...
    /*
    // Caller takes ownership. Returns all ranges intersecting [begin, end)
    std::vector<range_info> *range(target_ulong begin, target_ulong end) {
        std::vector<range_info> *results = new std::vector<range_info>;
        auto it = impl.upper_bound(begin); // first range past begin
        if (it != impl.begin()) it--; // 
        while (it != impl.end() && end > it->first) {
